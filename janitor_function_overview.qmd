...
[BONUS]{.emphasized} To make the code more tidy, you can change the parsing option, but it will override the `case=` argument.
...
* Can handle mixed types ("2024-10-01" vs "344402")   
...
characters_messy |> 
  ...
  adorn_totals(
    where = c("row", "col") #default is "row" only
  ) |> 
  adorn_title()
```
